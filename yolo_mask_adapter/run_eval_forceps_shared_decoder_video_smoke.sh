...
/root/anaconda3/bin/python -u yolo_mask_adapter/eval_dense_tracking_protocol.py \
  ...
  --mask-token-checkpoint yolo_mask_adapter/results/forceps_no_memory_decoder_autolabel_film_mlp_nomemattn_reinit_e8/best.pt \
  --model-checkpoint yolo_mask_adapter/results/forceps_shared_decoder_autolabel_film_mlp_joint_e4/best.pt \
  ...
  --device cuda:0 \
  --output-json yolo_mask_adapter/results/forceps_shared_decoder_video_yolotoken_10gt_smoke.json \
  2>&1 | tee yolo_mask_adapter/results/logs/forceps_shared_decoder_video_yolotoken_10gt_smoke.log
