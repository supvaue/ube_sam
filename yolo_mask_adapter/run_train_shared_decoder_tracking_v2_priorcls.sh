...
OUT_DIR=yolo_mask_adapter/results/shared_decoder_tracking_ligament_v2_priorcls_e3
...
echo "[stage] start shared-decoder tracking v2 prior-cls training"
...
/root/anaconda3/bin/python -u yolo_mask_adapter/train_shared_decoder_tracking.py \
  ...
  --target-class-id 1 \
  --output-dir "${OUT_DIR}" \
  --train-parts prompt_encoder mask_decoder \
  ...
  2>&1 | tee yolo_mask_adapter/results/logs/train_shared_decoder_tracking_ligament_v2_priorcls_e3.log
