...
/root/anaconda3/bin/python -u yolo_mask_adapter/eval_refined_init_tracking.py \
  ...
  --use-mask-as-output \
  --device cuda:0 \
  --output-json yolo_mask_adapter/results/forceps_refined_init_tracking_10gt_smoke.json \
  2>&1 | tee yolo_mask_adapter/results/logs/forceps_refined_init_tracking_10gt_smoke.log
